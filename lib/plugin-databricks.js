...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  ...
  if (getObj.operator) {
    ...
    } else if (getObj.operator === 'eq' && getObj.attribute === 'email') {
      // optional - only used when groups are member of users, not default behavior - correspond to getGroupUsers() in versions < 4.x.x
      path = `/Users?filter=userName eq "${getObj.value}"${(attributes.length > 0) ? '&attributes=' + attributes.join() : ''}`
    } else {
  ...
  try {
    ...
    if (Array.isArray(response.body.Resources)) responseArr = response.body.Resources
      ...
      else if (response.body.totalResults == 0) responseArr = []
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  var arrPhone = []
  var arrEntitlement = []
  if (Array.isArray(userObj.emails)) arrEmail = userObj.emails
    else {
      for (const key in userObj.emails) {
        arrEmail.push(userObj.emails[key])
      }
    } 
  if (Array.isArray(userObj.phoneNumbers)) arrPhone = userObj.phoneNumbers
    else {
      for (const key in userObj.phoneNumbers) {
        arrPhone.push(userObj.phoneNumbers[key])
      }
    } 
  if (Array.isArray(userObj.entitlements)) arrEntitlement = userObj.entitlements
    else {
      for (const key in userObj.entitlements) {
        arrEntitlement.push(userObj.entitlements[key])
      }
    } 

  scimgateway.logger.debug(`${pluginName}[${baseEntity}] handling "${action}" userObj=${JSON.stringify(userObj)} ents=${JSON.stringify(userObj.entitlements)} ems=${JSON.stringify(userObj.emails)}`)
...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  if (getObj.operator) {
    ...
    // mandatory - unique filtering - single unique user to be returned - correspond to getUser() in versions < 4.x.x
      if (getObj.attribute === 'id') path = `/Groups/${getObj.value}?attributes=${attributes.join()}` // path = `/Groups/${getObj.value}?attributes=${attributes.join()}` // GET /Users/bjensen?attributes=
...
scimgateway.modifyGroup = async (baseEntity, id, attrObj) => {
    ...
    if (config.entity[baseEntity].scimVersion && config.entity[baseEntity].scimVersion === '1.1') { // scim v1.1 endpoint
      ...
      if (addValues.length > 0) {
        body.Operations.push(
          {
            op: 'add',
            value: { members: addValues }
          }
        )
      }
      if (removeValues.length > 0) {
        removeValues.forEach(function (el) {
          body.Operations.push(
            {
              op: 'remove',
              path: `members[value eq "${el}"]`
            }
          )
        })
      }
  ...
  try {
    scimgateway.logger.debug(`${pluginName}[${baseEntity}] calling ${method} body = ${JSON.stringify(body)}`)
    const response = await doRequest(baseEntity, method, path, body)
    if (response.statusCode < 200 || response.statusCode > 299) {
      throw new Error(`${response.statusMessage} - ${JSON.stringify(response.body)}`)
    }
