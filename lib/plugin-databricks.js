...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  if (getObj.operator) {
    ...
    // mandatory - unique filtering - single unique user to be returned - correspond to getUser() in versions < 4.x.x
      if (getObj.attribute === 'id') path = `/Groups?filter=displayName eq "${getObj.value}"${(attributes.length > 0) ? '&attributes=' + attributes.join() : ''}` // path = `/Groups/${getObj.value}?attributes=${attributes.join()}` // GET /Users/bjensen?attributes=
