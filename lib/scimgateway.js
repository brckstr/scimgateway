...
const ScimGateway = function () {
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    try {
      const res = await this[handle.getMethod](ctx.params.baseEntity, utils.copyObj(getObj), ctx.query.attributes ? ctx.query.attributes.split(',').map(item => item.trim()) : ["groups","userName","displayName"])
...
//            res = await this[handler.users.getMethod](ctx.params.baseEntity, { attribute: 'id', operator: 'eq', value: getObj.value }, ['id', 'displayName'])
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`], async (ctx) => {
    ...
    try {
      ...
      for (const key in res) { // merge any result e.g: {'id': 'xxxx'}
        var vat = res[key]
        logger.debug(`${gwName}[${pluginName}] key "${key}" value "${vat}"`)
      ...
      if (!jsonBody.id) { // retrieve id
        ...
        logger.debug(`${gwName}[${pluginName}] special "${obj.Resources[0].id}" fred`)
        if (obj && obj.Resources[0].id) jsonBody.id = obj.Resources[0].id
      }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        // get current object
        logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
        let res = await this[handle.getMethod](ctx.params.baseEntity, { attribute: 'id', operator: 'eq', value: id }, ["displayName"])
